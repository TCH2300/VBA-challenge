...
'    last_row = Cells(Rows.Count, 1).End(xlUp).Row
    ...
    For Each ws In Worksheets
        ...
        ws.Cells(1, 10).Value = "Open Price"
            ws.Cells(1, 10).Font.Bold = False
        ws.Cells(1, 11).Value = "Closing Price"
            ws.Cells(1, 11).Font.Bold = False
        ...
        For i = 2 To last_row
            ...
            If ws.Cells(i - 1, 1).Value <> ticker Then
                ...
                ws.Cells(i_summary, 10).Value = open_price
            ...
            If ws.Cells(i + 1, 1).Value <> ticker Then 'if the ticker has changed, then populate the summary table and increment summary index
                close_price = ws.Cells(i, 6).Value
                ws.Cells(i_summary, 11).Value = close_price
                    Columns("K").NumberFormat = "_($* #,##0.00_);_($* (#,##0.00);_($*""_""??_);_(@_)"
                ws.Cells(i_summary, 9).Value = ticker
                ws.Cells(i_summary, 14).Value = volume_total
                    Columns("N").NumberFormat = "_($* #,##0_);_($* (#,##0);_($*""_""??_);_(@_)"
                volume_total = 0
                yearly_change = close_price - open_price
                ws.Cells(i_summary, 12).Value = yearly_change
                    Columns("L").NumberFormat = "_($* #,##0.00_);_($* (#,##0.00);_($*""_""??_);_(@_)"
                percent_change = (yearly_change / open_price)
                    Columns("M").NumberFormat = "0.00%"
                ws.Cells(i_summary, 13).Value = percent_change
                i_summary = i_summary + 1 'this means everytime the summary table populates, it moves down to the next row.
            End If
            
            If IsNumeric(ws.Cells(i_summary - 1, 13)) = True Then
                If percent_change >= 0 Then
                    ws.Cells(i_summary - 1, 13).Interior.Color = RGB(0, 255, 0)
                Else
                    ws.Cells(i_summary - 1, 13).Interior.Color = RGB(255, 0, 0)
                End If
            End If
